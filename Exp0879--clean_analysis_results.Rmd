...
paper1_cmpds_table%>% head()

# paper1_cmpds <- paper1_cmpds_table$compound
...
paper1_raw <- with_compound_col %>%
                      .[has_compound_conc_col] %>%
                      map(. , ~select(.x, c(condition, plate_number, compound, compound_conc, xtal_hit, batch, Temperature, value, value_norm )) ) %>%
...
# [1] 60 # including DMSO and ADPribose

###### All layouts for all paper-1 compounds --------------
paper1_layouts <- paper1_raw %>% 
                  select(condition, variable, plate_number, compound, compound_conc, xtal_hit, batch) %>%
                  distinct()
...
###### All Tmas for all paper 1 compounds --------------
paper1_tmas <- lapply(results_extracted, `[[`,  j = "model_tmas" ) %>%
                bind_rows() %>%
                ungroup() %>%
                select(condition, mean_tma1, mean_tma2, which_model) %>%
                rename(tma1 = "mean_tma1", tma2 = "mean_tma2") %>%
                inner_join(paper1_layouts, by = "condition")  
...
ggsave("draft_figures/Exp0879--20200914_tma_DRCs_model3_fixed.pdf", p_tma, width = 15, height = 15)
...
median_tma1 <-  quantile(paper1_tmas$tma1, 0.5) # 51.9
median_tma2 <- quantile(paper1_tmas$tma2, 0.5, na.rm = TRUE) # 61.2 

paper1_tmas %>% head()


### this should be added to dsfworld!!!
####
...
sorted_tmas <- paper1_tmas %>%
  filter(compound != 27) %>%
  pivot_wider(names_from = which_model, values_from = c(tma1, tma2))%>%

  pivot_longer(cols = c("tma1_s2_list", "tma1_s2_d_list", "tma2_s2_list", "tma2_s2_d_list"), names_to = "model_and_tma") %>%
  ...
  arrange(variable, two_sig_model) %>%
  ...
  rename_with( ~gsub( pattern = "new_", replacement = "", .x), starts_with("new_")) %>%
   pivot_longer(cols = tma1_s1_list:tma2_s2_list, names_to = "model_and_tma") %>%
    mutate(which_tma = str_sub(model_and_tma, start = 0, end = 4),
         two_sig_model = str_sub(model_and_tma, start = 6, end = -1)) %>% 
...
paper1_tmas # tma1 tma2 which_model
