...
compound_col_added <- dfs_protein_TAMRA %>% 
                      .[has_ZINC_col_only] %>%
                       map(. , ~left_join(.x, paper1_cmpds_table %>% select(ZINC, `Tube-label`))) %>%
                        map( . ,   ~rename( .x, compound = `Tube-label)) 

with_compound_col$Exp0827
...
#                       
# ## if there is only a ZINC column
# 
# ADPr <-  dfs_protein_TAMRA %>%
#             .[has_compound_conc_col] %>% 
#   map( dfs_protein_TAMRA ,
#                       ~filter(.x,  # for each tibble
#                               #across(any_of("compound") | any_of("ZINC"), # if there's a compound or ZINC column
#                                ~.x %in% c("ADPribose")) )) %>% # keep ADPr
